...
describe("testing withOptimizedConsumer HOC", () => {
  let ContextObj;
  beforeEach(() => (ContextObj = createContext({})));
  test("withOptimizedConsumer provides only desired context as well as other props", () => {
    const Child = withOptimizedConsumer(LightComponentPure, ContextObj, [
      "fibNum",
    ]);
    const wrapper = mount(<Child nonContextProp="someString" />, {
  ...
  test("MemoizedConsumer updates on prop changes", () => {
    const MemoizedConsumer = withOptimizedConsumer(
      LightComponentPure,
      ContextObj,
      ["fibNum"],
    );
    const Child = () => {
      ...
      return (
        <div id="click" onClick={() => update()}>
          <MemoizedConsumer nonContextProp={otherProp} />
        </div>
      );
    };
  ...
  test("MemoizedConsumer updates when connected context is changed", () => {
    ...
    const Child = () => {
      ...
      return (
        <div id="click" onClick={() => update()}>
          <MemoizedConsumer nonContextProp={"someString"} />
        </div>
      );
    };
    const wrapper = mount(<Child />, {
...
test("ensure using withOptimizedComponent saves computation time", () => {
  ...
  const MemoizedHeavyComponentPure = withOptimizedConsumer(
    HeavyComponentPure,
    ContextObj,
    ["fibNum"],
  );

  const MemoizedLightComponentPure = withOptimizedConsumer(
    LightComponentPure,
    ContextObj,
    ["fibNum"],
  );
  ...
  const wrapper2 = mount(
    <ProfilerWrapper
      Child={PureChild}
      Component={MemoizedLightComponentPure}
      componentIndex={1}
  ...
  const wrapper4 = mount(
    <ProfilerWrapper
      Child={PureChild}
      Component={MemoizedHeavyComponentPure}
      componentIndex={3}
